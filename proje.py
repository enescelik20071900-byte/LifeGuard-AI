...
import requests
import threading
import numpy as np # Hata veren logları durdurup, kendi görüntümüzü oluşturmak için lazım
...
try:
    ...
    print("CRITICAL: 'mediapipe.solutions' bulunamadı!")
    mp_pose = None
    mp_drawing = None
...
def generate_frames():
    ...
    while True:
        # Eğer kamera açılamadıysa (sunucuda olduğumuz için normal)
        if not cap.isOpened():
            # Hata mesajı yerine boş siyah bir ekran oluşturup döndür (logları kirletme)
            blank_frame = np.zeros((480, 640, 3), dtype=np.uint8)
            cv2.putText(blank_frame, "KAMERA ERISIMI YOK", (150, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            cv2.putText(blank_frame, "Sunucu Modu", (220, 280), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 200, 200), 1)
            
            ret, buffer = cv2.imencode('.jpg', blank_frame)
            yield (b'--frame\r\n' b'Content-Type: image/jpeg\r\n\r\n' + buffer.tobytes() + b'\r\n')
            time.sleep(1) # CPU yormasın
            continue
                ...
                if lm[0].visibility > 0.5:
                    ...
                    if avg_angle < KAFA_ACI_ESIGI:
                        ...
                        if elapsed >= 5 and not is_logged:
                            ...
                            if sys.platform == "win32": winsound.Beep(1000, 500)
                            is_logged = True
                    else:
                        is_suspicious = is_logged = False
                        start_time = 0
